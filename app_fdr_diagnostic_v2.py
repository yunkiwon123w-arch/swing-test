...
st.write("Streamlit 서버에서 FinanceDataReader를 통해 NAVER 한국주식 일봉을 받을 수 있는지 확인합니다.")
...
if st.button("NAVER 주가 연결 테스트", type="primary", use_container_width=True):
    ...
    try:
        ...
        if df is None or df.empty:
            ...
            st.write("FinanceDataReader 호출은 끝났지만 데이터가 비어 있습니다.")
        else:
        ...
        elapsed = time.time() - started
        st.error(f"4/4 NAVER 조회 실패 ({elapsed:.1f}초)")
        st.exception(e)
